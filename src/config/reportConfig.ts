...
export const getReportActionType = (slug: string): "buttons" | "dateRange" => {
  ...
  const hasDateRange =
    report.required.includes("fromDate") && report.required.includes("toDate");
...
export const generateReportNavigation = () => {
  ...
  Object.entries(REPORTS).forEach(([key, report]) => {
    ...
    categorizedReports[category].push({ ...report, key } as ReportOption & {
      key: string;
    });
  });
  ...
  return Object.entries(categorizedReports)
    .map(([categoryKey, reports]) => ({
      ...
      label:
        REPORT_CATEGORIES[categoryKey as keyof typeof REPORT_CATEGORIES]
          ?.label || categoryKey,
      order:
        REPORT_CATEGORIES[categoryKey as keyof typeof REPORT_CATEGORIES]
          ?.order || 999,
...
export const AVAILABLE_REPORTS_BY_LEVEL = {
  ...
  AGENT_CLERK: ["sales-report", "cash-report", "transaction-history-report"],
};
