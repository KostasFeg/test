...
function injectCSSVariables(cssVariables: string): void {
  ...
  console.log('💉 Injected CSS variables:', cssVariables.split('\n').length, 'variables');
}
 ...
 */
export function convertMasterConfigToDesignSystem(masterConfig: any): DesignSystemConfig {
  return {
    theme: {
      colors: {
      ...
      typography: masterConfig.theme?.typography ? {
        ...
        fontWeightSemibold: masterConfig.theme.typography.fontWeightSemibold,
...
export function getColorVariants(baseColor: string) {
  ...
  if (!tokens) return { light: baseColor, dark: baseColor, contrast: '#000000' };
